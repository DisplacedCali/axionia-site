...
/**
 * The double-count problem, made arithmetic.
 *
 * `REVIEW_GAPS` on the homepage states this in words — "nobody is asked
 * whether the fourth one overlaps the first three, so savings get counted
 * twice and nobody owns the arithmetic" — and until now nothing showed it.
 ...
 * conservative in the employer's favour — it assumes programs overlap only
 * as much as chance would predict, where in practice vendors target the
 * same high-cost members on purpose, so real overlap runs higher. Stating
...
/** Union of overlapping claims within one pool. Always ≤ the sum. */
...
export default function BenefitOverlap() {
  ...
  return (
    <div>
      ...
      <div className="grid lg:grid-cols-[1.35fr_1fr] gap-10 lg:gap-14 items-start">
        ...
        <div>
          ...
          <div className="mt-4 flex items-baseline gap-3">
            ...
            <span className="font-mono text-[11px] uppercase tracking-[0.12em] text-gray-cool">
              $ PMPM once the overlap is removed
            </span>
          </div>
          ...
          {active.length === 0 ? (
            <div className="mt-6 border border-border bg-base-2 px-5 py-4">
              <p className="text-[14px] leading-[1.65] text-gray-warm">
                ...
                its full claim, and every one after that overlaps something
                already in place.
              </p>
            </div>
          ) : (
          ...
          <div className="mt-7 border-l-2 border-blue pl-6 py-1">
            <p className="font-serif italic text-xl md:text-2xl leading-snug text-navy">
              ...
              The overlap only exists once you own all five.
            </p>
          </div>
        </div>
      </div>
      ...
      <p className="mt-8 text-[12px] leading-[1.6] text-gray-cool max-w-measure">
        ...
        {LIVES.toLocaleString("en-US")}. Overlap is modelled as the union of
        independent claims against each pool — a deliberately conservative floor,
        since vendors target the same high-cost members on purpose and real
        overlap typically runs higher. Yours will differ. The point is the
